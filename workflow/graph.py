...
class FinancialWorkflowGraph:
    def __init__(self, tools: List[BaseTool], llm, db_manager: DatabaseManager, ingestion_tool: DataIngestionTool):
    ...
    def execute_financial_task(self, state: AgentState) -> AgentState:
        ...
        # Always try to run RAG first, for any query
        enable_llm_fallback = state["context"].get("enable_llm_fallback", False)
        
        rag_response_text = self.ingestion_tool.get_top_chunks(last_message)
        
        if rag_response_text:
            rag_response = self.llm.invoke(
                f"Based on the following context, answer the user's question. If the context does not contain the answer, say 'I cannot answer this from the document.'\n\nContext: {rag_response_text}\n\nQuestion: {last_message}"
            )
            rag_response = rag_response.content
        else:
            rag_response = "I cannot answer this from the document."

        # Check the RAG response
        if "I cannot answer this from the document." in rag_response:
            if enable_llm_fallback:
                # Fallback to general LLM for an answer
                llm_response = self.llm.invoke(last_message)
                tool_results["rag_response"] = llm_response.content
                tool_results["source"] = "LLM_fallback"
            else:
                # Fallback is off, provide a specific "not found" message
                tool_results["rag_response"] = "I'm sorry, I could not find an answer to your question in the uploaded document."
                tool_results["source"] = "not_found"
        else:
            # RAG succeeded, so we use its answer and mark it as from the document.
            tool_results["rag_response"] = rag_response
            tool_results["source"] = "document"

        # If a RAG-based answer was found, we stop here. Otherwise, proceed to financial tools.
        if tool_results.get("source") in ["document", "LLM_fallback", "not_found"]:
            pass # The RAG response is the final result.
        else:
            # Execute original task logic if RAG did not provide a valid answer
            symbol_mapping = {
    ...
    def generate_response(self, state: AgentState) -> AgentState:
        ...
        source = tool_results.get("source")
        prefix = ""
        
        if source == "document":
            prefix = "**Answer from Document:**\n\n"
        elif source == "LLM_fallback":
            prefix = "**Answer from OpenAI:**\n\n"
        elif source == "not_found":
            final_response_content = "**Answer Not Found:**\n\nI'm sorry, I could not find an answer to your question in the uploaded document."
            return {
                **state,
        ...
        - The response should be based on the provided tool results.
        - Do not generate an answer if the source is 'not_found'.
        ...
        final_response_content = prefix + response.content
