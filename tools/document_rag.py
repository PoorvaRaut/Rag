...
class DocumentRAGTool(BaseTool):
    client: OpenAI = Field(default_factory=lambda: OpenAI(api_key=OPENAI_API_KEY), exclude=True)
    ...
    def call_openai_with_context(self, context, question):
        """Calls OpenAI API for chat completion with a specific context."""
    ...
    def call_openai_general(self, question):
        """Calls OpenAI API for a general answer without context."""
        if not OPENAI_API_KEY:
            logger.error("OPENAI_API_KEY is not set in environment.")
            return "❌ Error: API key not configured."
        try:
            ...
            return f"I couldn't find the answer in the document, but here's a general answer: {response.choices[0].message.content}"
        except Exception as e:
            logger.error(f"OpenAI API fallback error: {str(e)}")
            return f"I couldn't find the answer and an error occurred during LLM fallback: {e}"

    def _run(self, question: str, enable_llm_fallback: bool = False) -> str:
        """Synchronous RAG logic with fallback toggle."""
        if not doc_chunks:
            return "⚠️ No document uploaded. Please upload a PDF first."
        
        context = self.get_top_chunks(question)
        if context is None:
            context_answer = "unanswerable"
        else:
            context_answer = self.call_openai_with_context(context, question)

        if context_answer == "unanswerable":
            if enable_llm_fallback:
                return self.call_openai_general(question)
            else:
                return "I'm sorry, I could not find the answer to your question in the uploaded document."
        else:
            return context_answer

    async def _arun(self, question: str, enable_llm_fallback: bool = False) -> str:
        """Asynchronous version of the RAG logic."""
        return await asyncio.to_thread(self._run, question, enable_llm_fallback)